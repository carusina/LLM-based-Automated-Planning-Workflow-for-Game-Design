...
import './App.css';
import generateDummyPlan from './generateDummyPlan';

function App() {
  const bookRef = useRef();
  const [activeTab, setActiveTab] = useState('edit'); // list, view, edit, generate
  ...
  const handleGenerateContent = async (section) => {
    const generatedContent = generateDummyPlan(section);
    
    // 섹션에 따라 planData 업데이트
    if (section.includes('world')) {
      setPlanData({
        ...planData,
        worldBuilding: {
          ...planData.worldBuilding,
          [section.replace('world', '').toLowerCase()]: generatedContent
        }
      });
      ...
      setPlanData({
        ...
        characters: {
          ...planData.characters,
          [section.replace('character', '').toLowerCase()]: generatedContent
        }
      });
    } else if (section.includes('story')) {
      setPlanData({
        ...planData,
        story: {
          ...planData.story,
          [section.replace('story', '').toLowerCase()]: generatedContent
        }
      });
    } else if (section.includes('gameplay')) {
      setPlanData({
        ...planData,
        gameMechanics: {
          ...planData.gameMechanics,
          [section.replace('gameplay', '').toLowerCase()]: generatedContent
        }
      });
    } else if (section === 'artDirection') {
      setPlanData({
        ...planData,
        visualStyle: {
          ...planData.visualStyle,
          artDirection: generatedContent
        }
      });
    }
  };
  ...
  const handleSectionChange = (section) => {
    // 실제 페이지 번호 매핑 (표지 포함)
    const pageMap = {
      'basic': 1,    // 기본 정보는 1-2 페이지 (표지 다음)
      'world': 3,    // 세계관은 3-4 페이지
      'character': 5, // 캐릭터는 5-6 페이지
      'story': 7,    // 스토리는 7-8 페이지
      'gameplay': 9  // 게임플레이는 9-10 페이지
    };

    const targetPage = pageMap[section];
    console.log('Moving to section:', section, 'page:', targetPage);
    
    if (bookRef.current) {
      const book = bookRef.current.pageFlip();
      console.log('Current page before flip:', book.getCurrentPageIndex());
      book.flip(targetPage);
      console.log('Current page after flip:', book.getCurrentPageIndex());
    }
  };
  ...
  const renderPlanList = () => (
    <div className="plan-list">
      ...
      <div className="plan-grid">
        {plans.map((plan) => (
          <div key={plan.id} className="plan-card" tabIndex="0">
            ...
            <div className="plan-card-back">
              ...
              <div className="plan-actions">
                <button className="delete-btn" onClick={() => handleDeletePlan(plan.id)}>삭제</button>
              </div>
            </div>
          </div>
        ))}
